...
export class AuthService {
  ...
  async login(dto: LoginDto, correlationId?: string): Promise<AuthTokens> {
    ...
    if (!user) {
      this.logger.warn(
        { email: dto.email, correlation_id: correlationId },
    ...
    if (!valid) {
      this.logger.warn(
        { email: dto.email, user_id: user.id, correlation_id: correlationId },
        "login failure: invalid password",
      );
      throw new UnauthorizedException("Invalid email or password");
    }
    const primaryRole = await this.userRoleRepository.findPrimaryRoleByUserId(
      user.id,
    );
    ...
    const expiresIn = this.parseExpiryToSeconds(
      this.config.get<string>("JWT_ACCESS_EXPIRY", "1d"),
    );
    this.logger.info(
      {
        user_id: user.id,
  ...
  async refresh(
    ...
    try {
      ...
      this.logger.warn(
        { correlation_id: correlationId },
        "token generation failed: invalid refresh token",
      );
    ...
    return {
      ...
      expires_in: this.parseExpiryToSeconds(
